...
The public site needs **zero** env vars. The `/admin` Cover Letter Studio
(CV upload + LLM cover-letter generation, full docs in
[`docs/cv-cover-letter-flow.md`](docs/cv-cover-letter-flow.md)) is configured
entirely through `.env.local` (local) / Vercel env vars (prod):

| Variable | Required | What it does |
|---|---|---|
| `ADMIN_PASSWORD` | for `/admin` | Login password; without it the admin panel returns 503 |
| `ADMIN_SECRET` | recommended | Cookie-signing key (falls back to `ADMIN_PASSWORD`) |
| `DATABASE_URL` | prod only | `libsql://<db>.turso.io` — local dev defaults to `file:./data/portfolio.db` |
| `DATABASE_AUTH_TOKEN` | prod only | Turso auth token (ignored for file: URLs) |
| `LLM_PROVIDER` | no | `ollama` (default) \| `groq` \| `gemini` — pick the letter-writing backend |
| `GROQ_API_KEY` / `GROQ_MODEL` | groq mode | Key required; model defaults to `llama-3.3-70b-versatile` |
| `GEMINI_API_KEY` / `GEMINI_MODEL` | gemini mode | Key required; model defaults to `gemini-flash-latest` (rolling alias) |
| `OLLAMA_BASE_URL` / `OLLAMA_MODEL` | ollama mode | Default `http://localhost:11434/v1` / `qwen3` — local dev only |
| `OPENAI_API_KEY` | no | Optional bearer for custom OpenAI-compatible endpoints in ollama mode |
| `NEXT_PUBLIC_SKIN` | no | Build-time skin override (see Skins) |
| `NEXT_PUBLIC_SITE_URL` | no | Real domain for canonical URLs / SEO |

Rules of thumb:

- **After changing `.env.local`, restart the dev server** — Next.js reads it at boot.
- All providers speak the OpenAI-compatible API; switching is env-only, no code.
- If the LLM is unreachable or its key is missing, letters degrade to a built-in
  template (marked `source: "template"` and flagged in the admin UI) — the feature
  never hard-fails.
- Letter length (`short` ~140 words / `long` ~300) and audience (generic/targeted)
  are per-generation controls in the admin UI, not env config.
- `.env.local` is gitignored. Moving to a new machine: transfer it via AirDrop or
  a password-manager secure note — or point both machines at the same Turso DB and
  skip local state entirely (details in the flow doc).
...
Deploy. The public site needs no env vars; **the `/admin` panel does** — see
"Environment & configurables" above and the step-by-step Vercel checklist in
[`docs/cv-cover-letter-flow.md`](docs/cv-cover-letter-flow.md) (Turso DB + admin +
LLM keys). Every push to `main` auto-deploys; PRs get preview URLs.
...
| `/admin` | Cover Letter Studio — CV upload + LLM letters (password-gated) | **no** (noindex, undocumented) |
